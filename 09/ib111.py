...
def allow_modules():
    ...
    allow["threading"] = []
    allow["ib111"] = ["with_overrides_enabled"]
...
class file_proxy(StringIO):
    def __init__(self, fs, path, mode):
        self.fs = fs
        self.path = path
        self.mode = mode
        if mode.startswith("x") and path in fs:
            raise FileExistsError(path)
        self.ini = fs.setdefault(path, "")
        if mode.startswith("w"):
            self.ini = ""
        super().__init__(self.ini)
    ...
    def close(self):
        with overrides_disabled():
            ...
            fin = self.getvalue()
            super().close()
            if self.mode == "r" and fin != self.ini:
                raise PermissionError(self.path)
            self.fs[self.path] = fin
            self.fs = None
...
class fs_proxy:
    ...
    def _open(self, path: str, mode: str = "r", *_args, **_kwargs):
        return file_proxy(self.data, path, mode)
    ...
    def __enter__(self):
        self.orig = os.remove, os.path.exists, builtins.open
        os.remove = self._remove
        os.path.exists = self._exists
        builtins.open = self._open

    def __exit__(self, typ, val, tb):
        os.remove, os.path.exists, builtins.open = self.orig


global_fs_proxy = fs_proxy()


def wrap_open(attr, old, *args, **kwargs):
    with overrides_disabled():
        return global_fs_proxy._open(*args, **kwargs)
...
def make_proxy(mod_name, mod, allow):
    ...
    if mod_name == "os":
        ...
        mod.path.exists = global_fs_proxy._exists
        mod.remove = global_fs_proxy._remove
    ...
    if mod_name == "sys":
        wrap_attr(mod.stdout, "flush")
        wrap_attr(mod.stderr, "write")
        wrap_attr(mod.stderr, "flush")
        wrap_attr(mod.stdin, "read")
        wrap_attr(mod.stdin, "readline")
        wrap_attr(mod.stdin, "readlines")
...
def check_import():
    ...
    if line is None:
        _error(f"ib111 must be imported from the first line of your script")
        exit(1)

    matched = fullmatch(
        f"from ib111 import week_(..)" "(, except_data_structures|)(  # noqa|)\n", line
    )

    if matched is None:
        _error(
            f"wrong import of ib111: {line}"
